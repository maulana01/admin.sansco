...
export const meta: V2_MetaFunction = () => [{ title: 'Daftar Pesanan yang Sudah Selesai | Dashboard Koki' }];
...
export default function FinishedOrders() {
  ...
  useEffect(() => {
    const fetchFinishedOrder = async () => {
      const response = await fetch('https://mail.apisansco.my.id/api/v1/orders/finished-orders', {
        method: 'GET',
        headers: {
      ...
      setFinishedOrder(data.data);
      return data;
    };

    fetchFinishedOrder().then((data: any) => {
      setFinishedOrder(data.data);
    });
  ...
  return (
    <div style={main}>
      ...
      <div style={helper}>
        <h2 style={heading}>Daftar Pesanan yang Sudah Selesai</h2>
      </div>
      <div style={tableContainer}>
        <table style={table}>
          ...
          <tbody>
            {Array.isArray(finishedOrder) &&
              finishedOrder.map((order: any) => (
                <tr key={order.id}>
