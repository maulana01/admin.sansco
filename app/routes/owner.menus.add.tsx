...
export const meta: V2_MetaFunction = () => [{ title: 'Tambah Menu | Dashboard Owner' }];
...
export const action = async ({ request }: ActionArgs) => {
  ...
  if (body.get('name') === '' || body.get('price') === '' || body.get('category_id') === '' || body.get('image') === '') {
  ...
  const addMenu = async () => {
    try {
      ...
      formData.append('price', body.get('price') as string);
      formData.append('category_id', body.get('category_id') as string);
      formData.append('image', body.get('image') as Blob);

      const res = await fetch('https://mail.apisansco.my.id/api/v1/menus/', {
        method: 'POST',
        body: formData,
      });
      ...
      return { status: 'error', message: 'Gagal menambahkan Menu' };
    }
  };
...
export default function AddMenus() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (response && response.status === 'success') {
      notify('Data Menu berhasil ditambahkan!', 'success');
      setTimeout(() => {
        window.location.href = '/owner/menus/';
      }, 2500);
    } else {
      notify('Data Menu gagal ditambahkan!', 'error');
      setTimeout(() => {
        window.location.href = '/owner/menus/add';
      }, 2500);
    }
  };
  ...
  return (
    <div style={main}>
      <div style={helper}>
        <h2 style={heading}>Tambah Data Menu</h2>
      </div>
      {/* Form section */}
      <div style={formContainer}>
        <Form method='post' onSubmit={handleSubmit} encType='multipart/form-data'>
          ...
          <input style={formInput} type='number' id='dataInput' name='price' />
          <label style={formLabel} htmlFor='dataInput'>
            Kategori:
          </label>
          <select style={selectInputStyle} id='selectInput' name='category_id'>
            {loaderResult.data.rows.map((category: any) => (
