  ...
  if (status === 'Pesanan Sudah Dibayar') {
    const processOrder = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:4000/api/v1/orders/process-order/${params.order_code}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
    ...
    const finishOrder = async () => {
      try {
        // const email = useLoaderData();
        const res = await fetch(`http://localhost:4000/api/v1/orders/finish-order/${params.order_code}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
