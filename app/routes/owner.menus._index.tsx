...
export async function loader({ request }: LoaderArgs) {
  ...
  const fetchMenus = async () => {
    try {
      // const page = 1;
      // const limit = 10;
      const response = await fetch(`http://103.175.216.182:4000/api/v1/menus/`, {
        method: 'GET',
        headers: {
...
export async function action({ request }: LoaderArgs) {
  ...
  const deletemenu = async () => {
    try {
      const res = await fetch(`http://103.175.216.182:4000/api/v1/menus/${body.get('id')}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
            ...
            {menus.data.map((data: any) => (
              <tr key={data.id}>
                ...
                <div style={td}>
                  ...
                  <td>
                    {/* Pass the menu ID to setMenuIdToDelete */}
                    <button
                      style={buttonDelete}
                      onClick={() => {
