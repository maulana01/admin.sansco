...
export const meta: V2_MetaFunction = () => [{ title: 'Daftar Pengguna | Dashboard Owner' }];
...
export async function action({ request }: LoaderArgs) {
  ...
  const deleteUser = async () => {
    try {
      const res = await fetch(`https://mail.apisansco.my.id/api/v1/users/${body.get('id')}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return (
    <div style={modalOverlay}>
      <div style={modalContainer}>
        ...
        <p style={modalText}>Apakah Anda yakin ingin menghapus data user?</p>
...
export default function home() {
  ...
  const onDeleteConfirmed = async () => {
    ...
    if (response && response.status === 'success') {
      ...
      notify('Data user gagal dihapus!', 'error');
      setTimeout(() => {
        window.location.reload();
      }, 2500);
    }
